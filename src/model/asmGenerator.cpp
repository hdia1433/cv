...
void AsmGenerator::generateAssign(const Instruction& instr)
{
    ...
    std::string storeReg = OperandKind::reference == instr.arg1.kind ? "x28" : "w28";
...
void AsmGenerator::setVar(Variable* symbol, const std::string& storeReg, const std::string& useReg)
{
    std::string indent(indentNum, '\t');
    ...
    assembly << indent << "adrp " << useReg << ", _" << symbol->name << "@PAGE\n";
    assembly << indent << "add " << useReg << ", " << useReg << ", :lo12:_" << symbol->name << "@PAGEOFF\n";
    assembly << indent << "str " << storeReg << ", [" << useReg << "]\n";
}

void AsmGenerator::accessRef(Variable* symbol, const std::string& reg)
{
    std::string indent(indentNum, '\t');
    ...
    assembly << indent << "adrp " << reg << ", _" << symbol->name << "@PAGE\n";
    assembly << indent << "add " << reg << ", " << reg << ", :lo12:_" << symbol->name << "@PAGEOFF\n";
}

void AsmGenerator::accessDeref(Variable* symbol, const std::string& reg)
{
    std::string indent(indentNum, '\t');
    ...
    assembly << indent << "adrp " << xReg << ", _" << symbol->name << "@PAGE\n";
    assembly << indent << "add " << xReg << ", " << xReg << ", :lo12:_" << symbol->name << "@PAGEOFF\n";
    assembly << indent << "ldr " << xReg << ", [" << xReg << "]\n";
    assembly << indent << "ldr " << reg << ", [" << xReg << "]\n";
}

void AsmGenerator::setDeref(Variable* symbol, const std::string& storeReg, const std::string& useReg)
{
    std::string indent(indentNum, '\t');
    ...
    assembly << indent << "adrp " << useReg << ", _" << symbol->name << "@PAGE\n";
    assembly << indent << "add " << useReg << ", " << useReg << ", :lo12:_" << symbol->name << "@PAGEOFF\n";
    assembly << indent << "ldr " << useReg << ", [" << useReg << "]\n";
    assembly << indent << "str " << storeReg << ", [" << useReg << "]\n";
}
...
void AsmGenerator::generateOperation(const std::string& op, const Instruction& instr)
{
    ...
    if ("mul" == op)
    {
        if ('w' == arg1[0] && 'x' == arg2[0])
        {
            assembly << indent << "uxtw x" << arg1.substr(1) << ", " << arg1;
            arg1 = "x" + arg1.substr(1);
        }
        else if ('x' == arg1[0] && 'w' == arg2[0])
        {
            assembly << indent << "uxtw x" << arg2.substr(1) << ", " << arg2;
            arg2 = "x" + arg2.substr(1);
        }
    }
